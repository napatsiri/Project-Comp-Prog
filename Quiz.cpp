...
void student(vector<string> q,vector<string> a,vector<string> b, vector<string> c, vector<string> d, vector<string> ans){
	int numq = rand()%q.size();
	cout<<q[numq]<<"\n"<<a[numq]<<"\n"<<b[numq]<<"\n"<<c[numq]<<"\n"<<d[numq];
	cout<<"\n---------------------------------";
...
void quiz(string user){
    ...
    while(getline(read,textline)){
        char q[100],a[100],b[100],c[100],d[100],ans[1];
        sscanf(textline.c_str(),format,&q,&a,&b,&c,&d,&ans);
